    ...
    if usage:
        ...
        if not isRunningLinux():
            ...
            if not isUserAdmin():
                ...
                log.i("Starting kernel compiling")
                log.d("Checking versions")
                ...
                if version.parse(current_version) >= version.parse(new_version):
                    log.d("The version installed is the same or greater than the available one. Current version: " +
                    ...
                    log.d("Starting new version download... | New version: " + new_version)
                    ...
                    if compiler.copy_latest_config():
                        ...
                        log.d("Kernel compilation finished")
                        log.d("Starting kernel installation...")
                        print(Colors.OKBLUE + "Installing the new kernel..." + Colors.ENDC)
                        compiler.installKernel()
                        log.d("Finished correctly kernel installation. New version installed: " + new_version)
                        log.finish()
