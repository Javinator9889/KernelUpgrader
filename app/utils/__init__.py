...
def returnToHomeDir():
    import subprocess
    from app.values import GOTO_HOME
...
def isDEBSystem():
    # type: () -> bool
    import subprocess
    from app.values import RPM_OR_DEB
...
def removeOldKernels():
    import subprocess
    from app.values import CLEAN_KERNELS

    subprocess.run(CLEAN_KERNELS.split(), stderr=subprocess.PIPE, stdout=subprocess.PIPE)


def cleanupOldLogs():
    from app.values import FILE_PATH, FILENAME, COMPILER_FILENAME, TARFILE_FILENAME, TARFILE_COMPILER_FILENAME
    import tarfile
    import os
...
class Log:
    def __init__(self):
        from app.values import FILE_PATH, FILENAME
        cleanupOldLogs()
    ...
    class CompilerLog:
        def __init__(self):
            from app.values import FILE_PATH, COMPILER_FILENAME
            cleanupOldLogs()
